...
APHID_CLASSES = ('11121', '11122', '11111', '11123')


class Aphid4kAnnotationTransform(object):
    """This class converts the data from Annotation XML files to a list
    of [xmin, ymin, xmax, ymax, class]"""

    def __init__(self,
                 keep_difficult=False):
        """Class constructor for Aphid4kAnnotationTransform
        ...
        super(Aphid4kAnnotationTransform, self).__init__()
        self.class_to_index = dict(zip(APHID_CLASSES, range(len(APHID_CLASSES))))
...
class Aphid4k(Dataset):
    ...
    def __init__(self,
                 ...
                 mode,
                 image_transform,
                 target_transform=Aphid4kAnnotationTransform(),
                 keep_difficult=False):
        """Class constructor for Aphid4k
        ...
        with open(self.text_path.format(path, mode)) as f:
            for line in f:
...
def save_results(all_boxes,
    ...
    # for each class
    for class_i, class_name in enumerate(APHID_CLASSES):

        text = 'Writing {:s} APHID4k results file'.format(class_name)
...
def aphid_eval(detection_path,
    ...
    for image_name in image_names:
        target = [x for x in targets[image_name] if str(x['name']) == class_name]
...
def do_python_eval(results_path,
    ...
    aps = []
    for class_name in APHID_CLASSES:
        detection_path = osp.join(results_path, class_name + '.txt')
        recall, precision, ap = aphid_eval(detection_path=detection_path,
                                         path=path,
